...
| not filed | **open** | `protean server` with one worker calls `configure_logging(level=PROTEAN_LOG_LEVEL or "INFO")` before `Domain.init()`. Init then finds root handlers and skips `Domain.configure_logging`, so `[logging]` in `domain.toml` is never read. Multi-worker and `--reload` runs do read it | `regression/test_protean_regressions.py::test_server_single_worker_applies_domain_toml_logging_level` |
