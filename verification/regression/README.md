...
| [#1630](https://github.com/proteanhq/protean/issues/1630) | **open** | The `is_event_sourced` deprecation warning is attributed to `protean/domain/__init__.py` instead of the decorator that used the option (`stacklevel` one frame short) | `regression/test_protean_regressions.py::test_is_event_sourced_warning_points_at_the_decorator` |
...
| [#1635](https://github.com/proteanhq/protean/issues/1635) | **open** | `protean server` with one worker calls `configure_logging(level=PROTEAN_LOG_LEVEL or "INFO")` before `Domain.init()`. Init then finds root handlers and skips all of `Domain.configure_logging`: the `[logging]` level, redaction, the correlation processor and filter, the OpenTelemetry processor and `per_logger`. So engine logs carry no `correlation_id`. `protean observatory` has the same gap. `--reload` applies `[logging]` fully; `--workers N` applies it, but the supervisor's INFO listener drops records below INFO | `regression/test_protean_regressions.py::test_server_single_worker_applies_domain_toml_logging_level` |
