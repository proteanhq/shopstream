...
| structured logging + correlation processor | ⚠️ | API: `Domain.configure_logging` reads `[logging]` from `src/identity/domain.toml` (absent today, so Protean's defaults apply); `src/app.py` calls `configure_api_logging()` (`src/api_logging.py`) before any `init()`. `tests/test_logging_config.py` checks the config level, the `PROTEAN_LOG_LEVEL` override, the root correlation filter, the call order in `src/app.py`, and that no `domain.toml` declares `[logging]`, so Protean's defaults apply. Engines (`protean server`, single worker) apply the domain's `[logging]` since proteanhq/protean#1640 (#1635). `protean observatory` applies the first `--domain`'s `[logging]` (ordering, in `make observatory`). `verification/regression/test_protean_regressions.py::test_server_single_worker_applies_domain_toml_logging_level` guards the server path. No test covers the observatory path |
...
This branch pins Protean to git `main`, currently `e79a817` (after 0.17.0). #1023/#1025/#1028/
